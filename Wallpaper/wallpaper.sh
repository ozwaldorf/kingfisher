...
old_file=$(find $folder -maxdepth 1 -regex ".*/wallpaper.jpg" | head -n 1)
if [ -e "$old_file" ]; then
  rm "$old_file"
fi
...
#Add overlay
composite -gravity Center "$folder/assets/overlay.png" "$folder/$file_name" "$folder/$file_name"

#Set Wallpaper
gsettings set org.gnome.desktop.background picture-uri "'$folder'/wallpaper.jpg"
gsettings set org.gnome.desktop.screensaver picture-uri "'$folder'/wallpaper.jpg"
